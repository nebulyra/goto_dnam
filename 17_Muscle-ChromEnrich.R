##############################################################
# SCRIPT 17: Muscle - Chromatin enrichment
##############################################################
# Setup
rm(list=ls())
library(tidyverse)
...
for(i in chrom_states){
  res_road <- limma_base %>% 
    ...
    mutate(sig=ifelse(limma_base$cpg %in% top_cpgs$cpg, 1, 0),
           state=ifelse(E107 == i, 1, 0)) %>% 
...
write_csv(res %>% arrange(p, OR),
          file="../GOTO_Data/Tables/ST5-E107.csv")

for(i in chrom_states){
  res_road <- limma_base %>% 
    # Indicators for 5% FDR and state
    mutate(sig=ifelse(limma_base$cpg %in% top_cpgs$cpg, 1, 0),
           state=ifelse(E108 == i, 1, 0)) %>% 
    mutate(state=ifelse(is.na(state), 0, state))
...
# Suppl. Tab. 6
write_csv(res %>% arrange(p, OR),
          file="../GOTO_Data/Tables/ST5-E108.csv")
...
# Fig. 3a
png("../GOTO_Data/Chromatin_States/OR-Plot_Chromatin-muscle.png", 
...
write_tsv(homer, file='../GOTO_Data/Homer/Muscle_HomerInput.tsv')
